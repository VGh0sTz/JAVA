...
public class Operaciones {
    ...
    public void CrearPiloto(Piloto p1, Connection con) throws SQLException{
        ...
        String insercion="Insert Into drivers Values (?,?,?,?,?,?,?,?)"+" ON CONFLICT (driverid) DO NOTHING";
    ...
    //DONE
    public void CrearConstructor(Constructor c1, Connection con) throws SQLException{
        String insercion="Insert Into constructors Values (?,?,?,?,?)"+" ON CONFLICT (constructorid) DO NOTHING";
        try(PreparedStatement insercio=con.prepareStatement(insercion)){
            insercio.setInt(1,c1.getConstructorid());
            insercio.setString(2,c1.getConstructorref());
            insercio.setString(3,c1.getName());
            insercio.setString(4,c1.getNationality());
            insercio.setString(5,c1.getUrl());
            int id=insercio.executeUpdate();
        } catch (SQLException e) {
            e.getMessage();
            e.printStackTrace();
            throw new SQLException();
    ...
    //DONE
    public void get_results_by_driver(String code,Connection con){
        try(CallableStatement procedimiento = con.prepareCall("{CALL get_results_by_driver(?)}")){
    ...
    //DONE
    public void get_results_by_driver(ResultSet rs) {
        {
            try {
                while (rs.next()) {
    ...
    public static Piloto convertir_rs_a_piloto(ResultSet rs){
        ...
        try {
            while (rs.next()) {
                ...
                Piloto p1=new org.example.EJ6.Piloto(driverid,code,forename,surname,dob,nationality,constructorid,url);
                return p1;
            }
        }catch (Exception e){
            e.printStackTrace();
